...
from game.service import GameService, InvalidPromptError, NoImageError
...
def test_change_image_invalid_prompt(tmp_path, monkeypatch):
    ...
    svc.save_image_bytes("slack", "T1", "C1", b"base")
    with pytest.raises(InvalidPromptError) as exc:
        svc.change_image("slack", "T1", "C1", "add a bird")
...
def test_change_image_saves_new_image(tmp_path, monkeypatch):
    ...
    svc.save_image_bytes("slack", "T1", "C1", b"base")
    path = svc.change_image("slack", "T1", "C1", "add a bird")
